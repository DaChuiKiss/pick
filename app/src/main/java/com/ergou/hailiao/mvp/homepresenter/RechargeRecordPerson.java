...
public class RechargeRecordPerson extends RxPresenter<RechargeRecordContract.MainView>
        implements RechargeRecordContract.Presenter {
    private RetrofitUtil mRetrofitHelper;
    private Activity activity;

    @Inject
    public RechargeRecordPerson(RetrofitUtil mRetrofitHelper, Activity activity) {
    ...
    @Override
    public void getRechargeRecordBean(RequestBody body) {

        addSubscrebe(mRetrofitHelper.startObservable(mRetrofitHelper.getApiService().getChongzhi(body),
                new ResourceSubscriber<HttpResponse<List<RechargeRecordBean>>>() {
                    @Override
                    public void onNext(HttpResponse<List<RechargeRecordBean>> response) {
                        LogUtils.e("=========充值列表返回：" + response.getData().toString());
                        if (response.getCode() == 200) {
                            mView.get().getRechargeRecordTos(response.getData());
                        } else {
                            mView.get().showError();
