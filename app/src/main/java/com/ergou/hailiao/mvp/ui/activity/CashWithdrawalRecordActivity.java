...
public class CashWithdrawalRecordActivity extends BaseActivity<CashWithdrawlRecordPerson>
        implements CashWithdrawlRecordContract.MainView {
    ...
    protected void initInject() {
        getActivityComponent().inject(CashWithdrawalRecordActivity.this);
    }
    ...
    protected void initEventAndData() {
        titleShare.setText(R.string.cash_withdrawal_record);
    }
    ...
    private void init() {
        ...
        multiTypeAdapter.register(CashWithdrawalRecordBean.class, cashWithdrawalRecordAdapter);
...
//                refreshBoolean = false;
    ...
    public void getCashWithdrawalRecord() {//获取提现记录
        ...
        RequestBody requestBody = build.build();
        mPresenter.getCashWithdrawalRecordBean(requestBody);
    }
    ...
    @Override
    public void getCashWithdrawalRecordTos(List<CashWithdrawalRecordBean> cashWithdrawalRecordBeans) {//提现记录
        refresh.setRefreshing(false); // 关闭动画也就是圈圈消失
        items.clear();
        if (cashWithdrawalRecordBeans.size() != 0) {
            whether.setVisibility(View.GONE);
            items.addAll(cashWithdrawalRecordBeans);
            multiTypeAdapter.notifyDataSetChanged();
        } else {
            whether.setVisibility(View.VISIBLE);
        }
