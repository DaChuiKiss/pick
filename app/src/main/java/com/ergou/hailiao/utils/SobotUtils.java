...
public class SobotUtils {
    ...
    public static void startSobot(Context context) {
        ...
        customerFields.put("weixin", "ergou");
        customerFields.put("weibo", "ergouzi");
        customerFields.put("userSex", "男");
...
//        String appkey = SobotSPUtil.getStringData(context, "sobot_appkey", "");
        String appkey = "bb85ba9496ad4e5a9e9e848a5288dd77";
        if (!TextUtils.isEmpty(appkey)) {
            info.setAppkey(appkey);

            //设置标题显示模式
            SobotApi.setChatTitleDisplayMode(context,
                    SobotChatTitleDisplayMode.values()[enumType], "");
            //设置是否开启消息提醒
            SobotApi.setNotificationFlag(context, false
                    , R.drawable.dialogue_n, R.drawable.dialogue_y);
            SobotApi.hideHistoryMsg(context, sobot_show_history_ruler);
            SobotApi.setEvaluationCompletedExit(context, false);
            //启动帮助中心
//            SobotApi.openSobotHelpCenter(context, info);
            //启动聊天界面
